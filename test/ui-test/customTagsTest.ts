  ...
  describe("Verify extension's custom tags", () => {
    ...
    it('YAML custom tags works as expected', async function () {
      ...
      await driver.wait(
        ...
        10000,
        'Suggest widget did not become visible'
      );
      ...
      if (!items.includes('customTag1')) {
        expect.fail(
          "The 'customTag1' custom tag did not appear in the content assist's suggestion list. Found: " + items.join(', ')
        );
      }
